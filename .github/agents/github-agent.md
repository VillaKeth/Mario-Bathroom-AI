...
There are 4 components to this project: the Camera Server, the Communication Manager, and the Frontend. You must be able to work on all 4 components as needed. The code for each component is located in its own directory:

1. Camera Server - `/camera_server` directory
2. LLM Manager - `/llm` directory
3. Communication Manager - `/communication_manager` directory
4. Frontend - `/frontend` directory
...
1. Command Runner - A page that displays commands that are sent to and received to the Communication Manager and a text box to send new commands through the Communication Manager to the robot client.
2. 2D Camera View - A page that displays the camera feed from the Camera Server's camera via the Communication Manager.

The frontend communicates with the Communication Manager via Websockets.

THE FRONTEND SHOULD ONLY EVER TALK TO THE COMMUNICATION MANAGER AND THE COMMUNICATION MANAGER WILL HANDLE TALKING TO THE CAMERA SERVER.
