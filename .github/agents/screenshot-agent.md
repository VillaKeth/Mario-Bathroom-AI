...
You must actually run and test the entire application to verify that it works as expected. Ensure to run all components of the application in separate terminals and open this project in the browser to verify everything is working as expected.

# Overview

There are 7 components to this project: the Camera Server, the Communication Manager, and the Frontend. You must be able to work on all 7 components as needed. The code for each component is located in its own directory:
...
7. Doosan Robot Application - `/Villa DART Studio Task` directory

ONLY LOOK AT AND EDIT FILES IN THESE DIRECTORIES.

You also have permission to view anything in the `.github` directory, the `config.yaml` file in the root directory and the `TODO.md` file in the root directory.
...
This is a Protobuf-based microservice responsible for defining commands that are sent and received from the Doosan robot.
...
This is a microservice responsible for running tests on the other microservices and the frontend. It should be able to run tests in parallel and report results.

## Doosan Robot Application

The application is written in DRL (Doosan Robot Language) code, a Python-based programming environment. It is based on Python 3.2.3 syntax but it has some additional libraries and functions specific to Doosan robots.
...
2. Communication Manager: `cd communication_manager; npm run dev`
3. LLM Manager: `cd llm; python __main__.py`
...
When opening the Chrome browser, you must verify that there are NO CONSOLE ERRORS. If there are console errors, you must fix ALL of them.

You must also verify that the application looks visually correct, and is responsive on all devices and viewport sizes.

Your work is not finished until the application is responsive, looks correct on all devices and viewport sizes, there are no console errors and the application behaves properly.